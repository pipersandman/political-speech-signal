...
CANON_MAP = {
    # normalize common groups/aliases → canonical label
...
def canon_label(text: str, label: str) -> str:
    t = text.lower().strip()
    if t in CANON_MAP:  # manual overrides
        return CANON_MAP[t]
    # NORP/ORG/GPE → title-case text as default canonical
    return text.strip()
...
def intensity_feats(sent_text: str) -> dict:
    ...
    praise_hits = len(PRAISE & toks)
    attack_hits = len(ATTACK & toks)
    exclam = 1 if EXCLAM.search(sent_text) else 0
    allcap = 1 if ALLCAP.search(sent_text) else 0
    return {"praise": praise_hits, "attack": attack_hits, "exclam": exclam, "allcap": allcap}
...
def main():
    ...
    # top praised/criticized by avg sentiment (min mention threshold)
    MIN_M = 5
    praised = sorted([r for r in rows if r["mentions"] >= MIN_M], key=lambda r: (r["avg_sentiment"], r["mentions"]), reverse=True)[:10]
    criticized = sorted([r for r in rows if r["mentions"] >= MIN_M], key=lambda r: (r["avg_sentiment"], -r["mentions"]))[:10]
