...
try:
    from transformers import pipelineimport os, json, re, argparse, pathlib, csv, sys
from typing import List, Dict, Any, Optional, Iterable
import pandas as pd
from tqdm import tqdm
from concurrent.futures import ThreadPoolExecutor, as_completed

# Silence HF pipeline warnings
try:
...
def make_user_prompt_multi(batch_posts: List[Dict[str, Any]]) -> str:
    """
    Ask the model to return a JSON ARRAY where each element matches the schema and includes the same id.
    """
...
def adjudicate_batch_ollama(model: str, batch_posts: List[Dict[str, Any]], options: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # fallback: triage only for the whole batch
        rows = []
        for p in batch_posts:
...
def main():
    ...
    ap.add_argument("--checkpoint_every", type=int, default=5, help="Write CSV every N completed batches")
    ...
    # Make batches of indices_for_llm
    batches = []
