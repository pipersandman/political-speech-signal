...
def any_pattern(text, patterns): import re; return any(re.search(p, text, flags=re.I) for p in patterns)
def any_term(text, terms): t=text.lower(); return any(term in t for term in terms)
...
def make_zeroshot():
    if not USE_TRANSFORMERS: return None
    try: return pipeline("zero-shot-classification", model="facebook/bart-large-mnli")
    except Exception: return None

ZSC = None
def zeroshot_triage(text: str) -> Dict[str,bool]:
    global ZSC
    if ZSC is None: ZSC = make_zeroshot()
    if ZSC is None: return {}
    labels = [
    ...
    res = ZSC(text, labels, multi_label=True)
    out={}
...
def coalesce_text(r): 
    for k in ["text","content","body","message"]:
        if k in r and isinstance(r[k],str) and r[k].strip(): return r[k]
    return ""
def coalesce_id(r,i): 
    for k in ["id","post_id","uuid"]:
        if k in r and str(r[k]).strip(): return str(r[k])
    return f"row_{i}"
...
def load_any(path: pathlib.Path) -> List[Dict[str,Any]]:
    ...
    if path.suffix.lower()==".jsonl":
        ...
        df=pd.read_csv(path); items+=df.to_dict(orient="records")
    return items
...
def load_posts(input_path: str) -> List[Dict[str,Any]]:
    ...
    records=[]
    for fp in files: records+=load_any(fp)
    posts=[]
    for i,r in enumerate(records):
        text=coalesce_text(r)
        if not text.strip(): continue
        posts.append({"id":coalesce_id(r,i),"created_at":coalesce_created_at(r),
                      "url":r.get("url",""),"text":text.strip()})
    return posts
...
def main():
    ...
    ap.add_argument("--adjudicate", choices=["all","triaged"], default="triaged")
    args=ap.parse_args()
    ...
    md=["# One-off Rhetoric Analysis Summary\n","## Totals\n"]
    for k,v in report["totals"].items(): md.append(f"- {k}: {v}")
